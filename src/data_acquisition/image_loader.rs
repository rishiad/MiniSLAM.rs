...
pub fn stero_img_loader(video_file: &str) -> Result<Vec<(Option<i64>, image::GrayImage)>, Box<dyn std::error::Error>> {
    ...
    for (stream, packet) in format_input.packets() {
        if stream.index() == input_stream_index {
            ...
            event!(Level::INFO, "Decoding video frame with timestamp: {:?}", timestamp);
            ...
            if frame_width == 0 || frame_height == 0 {
                event!(Level::INFO, "Skipping frame with timestamp: {:?}", timestamp);
                continue;
            }
            ...
            event!(Level::INFO, "Decoded video frame with timestamp: {:?}", timestamp );
        }
    }
